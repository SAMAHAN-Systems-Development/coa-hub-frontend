...
export interface SkeletonBoxProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "children"> {
  w?: number | string;
  h?: number | string;
...
const sizeMap: Record<SkeletonSize, {
    gap: string;
    padding: string;
    lineHeight: string;
    lineRadius: string;
    ...
    sm: { gap: "gap-2", padding: "p-4", lineHeight: "h-2.5", lineRadius: "rounded", avatar: "h-8 w-8", mediaHeight: "h-28", titleHeight: "h-3" },
    md: { gap: "gap-3", padding: "p-5", lineHeight: "h-3", lineRadius: "rounded-md", avatar: "h-10 w-10", mediaHeight: "h-40", titleHeight: "h-3.5" },
    lg: { gap: "gap-4", padding: "p-6", lineHeight: "h-3.5", lineRadius: "rounded-md", avatar: "h-12 w-12", mediaHeight: "h-56", titleHeight: "h-4" },
    xl: { gap: "gap-6", padding: "p-8", lineHeight: "h-4", lineRadius: "rounded-lg", avatar: "h-14 w-14", mediaHeight: "h-72", titleHeight: "h-5" },
}
...
export function SkeletonText({
    ...
    const widths = lineWidths ?? (lines === 1 ? ["70%"] : ["100%", "85%", "60%", ...Array(Math.max(0, lines - 3)).fill("90%")]);
...
export function SkeletonCard({
    ...
    return (
        <Card className={cn("bg-card text-card-foreground overflow-hidden", className)} {...props}>
            {/* Image Section */}
