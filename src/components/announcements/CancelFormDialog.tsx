    ...
    <AlertDialog open={isOpen} onOpenChange={onCancel}>
      <AlertDialogContent className="bg-gradient-to-r from-[#373C44] to-[#6C7178] border-none text-white w-[85vw] max-w-[320px] sm:min-w-[750px] sm:w-auto sm:max-w-[90vw] px-6 py-6 sm:px-20 sm:py-8 rounded-2xl">
        <AlertDialogHeader className="space-y-3 sm:space-y-6">
          ...
          >
            {mode === "create" ? "CANCEL NEW POST?" : "CANCEL EDITING?"}
          </AlertDialogTitle>
          <AlertDialogDescription
          ...
          >
            ...
            {mode === "create" ? "creating the new post" : "editing"}?
            <br />
            New information will not be saved.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="flex flex-row justify-center gap-3 sm:gap-8 pt-4 sm:pt-8 pb-1 sm:pb-2 sm:justify-center">
          <AlertDialogAction
            onClick={onConfirm}
            className="bg-gradient-to-r from-[#6C7178] to-[#49515A] hover:from-[#373c44] hover:to-[#373c44] text-white px-3 sm:px-5 py-2 sm:py-3 text-sm sm:text-base rounded-sm border-none h-auto font-normal w-[80px] sm:w-[120px] transition-all"
            style={{ fontFamily: "Montserrat, sans-serif" }}
          >
            Cancel
          </AlertDialogAction>
